...
const placeholderExamples = [
    "A simple, classic wooden chest with iron bands.",
    "An ornate golden chest, encrusted with glowing gems.",
    "A spooky, mimic chest with sharp teeth and a slimy tongue.",
    "A futuristic sci-fi crate with neon lights.",
    "An ancient stone sarcophagus sealed with magical runes.",
];
...
const HistoryPanel: React.FC<{ history: AssetRecord[], onSelect: (item: AssetRecord) => void, disabled: boolean }> = ({ history, onSelect, disabled }) => (
  <div className="mt-6 border-t-2 border-gray-700 pt-4">
    ...
    {history.length === 0 ? (
      <p className="text-gray-500">Your generated chests will appear here.</p>
    ) : (
...
const TreasureChestGenerator: React.FC<GeneratorProps> = ({ apiLock }) => {
  ...
  const handleAdjust = useCallback(async () => {
    ...
    try {
      ...
      setGeneratedImage(imageDataUrl);
      setAdjustmentPrompt('');
      await addAsset({ type: 'chest', prompt: `Adjusted: ${adjustmentPrompt} (Original: ${prompt})`, imageDataUrl });
      loadHistory();
    } catch (err) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="bg-gray-800 p-6 rounded-lg shadow-2xl border-2 border-gray-700">
        <h2 className="text-2xl text-yellow-400 mb-4 font-press-start">1. Describe Treasure Chest</h2>
        <p className="text-gray-300 mb-4 text-lg">Describe a chest. The AI will generate a 3-frame sprite sheet (144x48px) for an opening animation.</p>
        
        <textarea
          value={prompt}
          onChange={(e) => setPrompt(e.target.value)}
          placeholder="e.g., A weathered wooden chest with a skull lock."
        ...
        <Button onClick={handleGenerate} disabled={apiLock.isApiLocked || !prompt} className="mt-4 w-full">
          {isLoading ? 'Generating...' : 'Generate Chest'}
        </Button>
      </div>
      <div className="bg-gray-800 p-6 rounded-lg shadow-2xl border-2 border-gray-700 flex flex-col">
        <div className="flex-grow">
          ...
          <SpriteDisplay
            ...
            loadingText={isAdjusting ? 'Remaking treasure...' : 'Forging your treasure...'}
            placeholder={
              <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
                  ...
                  <p className="text-xl">Your treasure chest sprite will appear here.</p>
              </div>
            }
            downloadFileName={'treasure_chest.png'}
            imageAlt="Generated treasure chest sprite"
