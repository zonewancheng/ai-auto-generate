...
const placeholderExamples = [
    "A small, fluffy, white wolf cub companion.",
    "A miniature dragon that sits on your shoulder.",
    "A majestic white horse with a leather saddle.",
    "A cute, floating slime with big eyes.",
    "A powerful armored bear mount.",
...
const HistoryPanel: React.FC<{ history: AssetRecord[], onSelect: (item: AssetRecord) => void, disabled: boolean }> = ({ history, onSelect, disabled }) => (
  <div className="mt-6 border-t-2 border-gray-700 pt-4">
    ...
    {history.length === 0 ? (
      <p className="text-gray-500">Your generated pets will appear here.</p>
    ) : (
...
const PetGenerator: React.FC<GeneratorProps> = ({ apiLock }) => {
  ...
  const handleAdjust = useCallback(async () => {
    ...
    try {
      ...
      setGeneratedImage(imageDataUrl);
      setAdjustmentPrompt('');
      await addAsset({ type: 'pet', prompt: `Adjusted: ${adjustmentPrompt} (Original: ${prompt})`, imageDataUrl });
      loadHistory();
    } catch (err) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="bg-gray-800 p-6 rounded-lg shadow-2xl border-2 border-gray-700">
        <h2 className="text-2xl text-yellow-400 mb-4 font-press-start">1. Describe Pet or Mount</h2>
        <p className="text-gray-300 mb-4 text-lg">Describe a companion. The AI will generate a 3x4 walking sprite sheet (144x192px).</p>
        
        <textarea
          value={prompt}
          onChange={(e) => setPrompt(e.target.value)}
          placeholder="e.g., A tiny fairy with glowing wings."
        ...
        <Button onClick={handleGenerate} disabled={apiLock.isApiLocked || !prompt} className="mt-4 w-full">
          {isLoading ? 'Taming Beast...' : 'Generate Pet'}
        </Button>
      </div>
      <div className="bg-gray-800 p-6 rounded-lg shadow-2xl border-2 border-gray-700 flex flex-col">
        <div className="flex-grow">
          ...
          <SpriteDisplay
            ...
            loadingText={isAdjusting ? 'Retraining companion...' : 'Bringing companion to life...'}
            placeholder={
              <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
                  ...
                  <p className="text-xl">Your pet/mount sprite will appear here.</p>
              </div>
            }
            downloadFileName={'pet_sprite.png'}
            imageAlt="Generated pet sprite sheet"
